    ...
    if (ref) { // =>{ type: mongoose.Schema.Types._id, ref: 'ActionType' }
      fields.push(
        <Reference url={ref} key={key}>
          {({ data, onSearchValueChanged, loading, onFocus }) => <Select data={data} optionLabel={optionLabel} onSearchValueChanged={onSearchValueChanged} loading={loading} name={key} label={startCase(key)} onFocus={onFocus} />}
        </Reference>,
      );
      return;
    }
    ...
    if (isValueArray && value[0] && value[0].ref) { // => [{ type: mongoose.Schema.Types._id, ref: 'Vertical'}]
      fields.push(
        <Reference url={value[0].ref} key={key}>
          {({ data, onSearchValueChanged, loading, onFocus }) => <MultiSelect data={data} onSearchValueChanged={onSearchValueChanged} loading={loading} name={key} label={startCase(key)} onFocus={onFocus} />}
        </Reference>,
      );
      return;
    }
